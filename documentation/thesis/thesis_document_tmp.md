...
: Build hardware sensors with equipped with different sensor (+ic)s to cover different magnet charaterisation setups \label{Build_hardware_sensors_with_equipped_with_different_sensor_(+ic)s_to_cover_different_magnet_charaterisation_setups.csv}
...
![Manufactured 1D sensor construction from different angles and universal magnet mount (green 3D printed part) populated with N45 12x12x12mm magnet \label{Manufactured_1D_sensor_construction_from_different_angles_and_universal_magnet_mount_(green_3D_printed_part)_populated_with_N45_12x12x12mm_magnet.png}](./generated_images/border_Manufactured_1D_sensor_construction_from_different_angles_and_universal_magnet_mount_(green_3D_printed_part)_populated_with_N45_12x12x12mm_magnet.png)
...
![3D plot of an N45 12x12x12mm magnet using the build 3D full-sphere sensor with visible magnetic field strength deviations using different shaded vertices on a sphere \label{3D_plot_of_an_N45_12x12x12mm_magnet_using_the_build_3D_full-sphere_sensor_with_visible_magnetic_field_strength_deviations_using_different_shaded_vertices_on_a_sphere.png}](./generated_images/border_3D_plot_of_an_N45_12x12x12mm_magnet_using_the_build_3D_full-sphere_sensor_with_visible_magnetic_field_strength_deviations_using_different_shaded_vertices_on_a_sphere.png)
...
![MRP library implemented modules high level overview each of which can be replaced by the user with custom ones \label{MRP_library_implemented_modules_high_level_overview_each_of_which_can_be_replaced_by_the_user_with_custom_ones.png}](./generated_images/border_MRP_library_implemented_modules_high_level_overview_each_of_which_can_be_replaced_by_the_user_with_custom_ones.png)
...
The various steps are linked by using the *stage <name>* makro as input parameter of the next function to be executed (see comments in listing \ref{lst:mrpuddp_example_yaml}).
...
After several simplification steps, determination of possible start steps and repeated traversal, the final execution sequence can be determined in the form of a call tree in figure \ref{Example_result_of_an_step_execution_tree_from_user_defined_processing_pipeline.png}.
...
Software tests in libraries offer numerous advantages for improving quality and efficiency. Software tests ease to identify errors and vulnerabilities before the software is released as a new version.
...
The (+cli) scripts are written in Python and simplifiys the execution for the user since there ist no *python3* prefix needed. This has been configured in the installation configuration using the *entry_points* option, and the following commands are available to the user:
...
Another possibility for filterin is to use a reference measurement instead of a simulated ideal magnet as a reference, as described before.
This can initialy come from a magnet selected as a reference magnet.
...
The figure \ref{MRP_evaluation_result_after_execution_of_the _user_defined_pipeline,_using_find_similar_values_algorithm.png} summarizes this result.
...
![Sensor evaluation setup for linearity measurements \label{Sensor_evaluation_setup_for_linearity_measurements.png}](./generated_images/border_Sensor_evaluation_setup_for_linearity_measurements.png)
...
In the following, alternative methods for sensor adaptions will be described that the sensors can still be used for these measurement.
