...
As it is possible to connect many identical sensors to one host using the `Network-Proxy`\ref{network-proxy}, it must be possible to address them separately. This separation is done by the `MRPProxy.proxy` module and is therefore not part of the core (+mrp) library.
