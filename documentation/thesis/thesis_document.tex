...
most \gls{mri} machines, providing in general a higher image contrast.
The substantial costs, space requirements and safety and installation
...
\tightlist
\item
  \gls{mrp}-library, explained in chapter
...
several steps, the procedure described by the following sub-chapters.
...
measurement and meta-data. The next step is to record and store real
sensor data. For this purpose, the \emph{MRPHal} module is developed,
which can interact with all \emph{Unified Sensor} compatible sensors
(refer to chapter \ref{unified-sensor}). In the following example
...
seen that of the initial \emph{10} measurements (upper plot), only four
measurements that are most similar to the \emph{\gls{cog}} mean value
were filtered (lower plot). The number of desired result measurements
was defined here by the parameter \emph{IP\_return\_count}. It can be
seen from the plotted \emph{\gls{cog}} \(\mu\)T mean values values, that
these are closest to an ideal Magnet with a \emph{\gls{cog}} value of
0\(\mu\)T. This ideal value is calculated with the function
...
executed via the user-programmable pipeline The calculation result was
...
implemented solution were demonstrated in their functional capability.
...
measuring ranges. To ensure comparability, the ideal curve is determined
\customcite{CSG22}. In order to be able to make quantifiable statements
about the measurement results, the mean and \gls{sd} deviation of these
two curves is determined.
...
DR & {[}-178;220{]} & {[}-3.1;3.4{]} & \(\mu\)T & maximum linear
\gls{dr}\tabularnewline
\bottomrule
\end{longtable}
...
\gls{dr} of \emph{±150mT}, but does not achieve the required accuracy
