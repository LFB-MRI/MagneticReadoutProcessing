...
evaluate several sensors in a shorter time, as a separate \gls{pcb} has
to be produced here.
...
characterisation to the choosen digital sensors
\ref{Digital_magnetic_field_sensors_characterised_for_evaluation.csv}.
...
and thus corresponds to the same conditions as in the \emph{Background
Noise} chapter \ref{sensor-characterisation-background-noise} setup.
...
towards zero) are decisive. The previously performed \emph{Background
Noise} characterisation in chapter
...
The field of permanent magnets is high temperature-dependent and can
...
DR & {[}-138;220{]} & {[}-3.1;3.4{]} & \(\mu\)T & maximum linear
...
the large \gls{dr}, which fulfils the required specifications from
