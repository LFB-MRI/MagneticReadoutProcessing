...
method plays a central role in contemporary medicine and research,
...
\customcite{OTW19} have already demonstrated low-cost and small-scale
...
\customcite{OTW19} in 2019.This innovative setup is subsequently employed
in 2020 to acquire in vivo MR images, showcasing the practical
applications of their pioneering research \customcite{OTW21}. In 2023, de
Vos, Remis and Webb published a summary of the design of a point-of-care
Halbach array low-field \gls{mri} system \customcite{DRW23}.
...
diameter Bore Halbach Array magnet'' \customcite{OTW19} in 2019. This
innovative setup is subsequently employed in 2020 to acquire in vivo MR
images, showcasing the practical applications of their pioneering
research \customcite{OTW21}. In 2023, de Vos, Remis and Webb published a
summary of the design of a point-of-care Halbach array low-field
\gls{mri} system \customcite{DRW23}.
...
homogeneity, like a genetic algorithms which O'Reilly implemented
...
field homogeneity, the unique goal at this point is to characterize the
magnets pre-installation, allowing for proactive assessments of
homogeneity characteristics. This approach aims to provide insights into
magnetic field uniformity before the magnets are integrated, thereby
streamlining the optimisation process. Furthermore,
...
The following chapter defines possible use cases that the future project
need to cover. These use cases define the setup of the hard- and
...
The use cases are defined in the course of project planning and provide
...
target group and designing the end result accordingly.
...
It simplifies of own measuring devices into the \gls{mrp} ecosystem at a
later date. This is only applicable with a minimal functional hardware
and firmware setup; it is developed for this purpose in the first step.
...
focisses on the most common and cost-effective ones, especially those
...
table
\ref{List_of_in_unified_firmware_implemented_digital_magnetic_field_sensors_with_focus_on_different_sensor_capabilities.csv}
with regard to their sensitivity and other parameters. This is done at
this point, as the components of the readout interface that enable
...
All parts are produced using 3D printing additive manufacturing proces.
...
The exchangeable magnetic holder (shown in green in figure
\ref{Rendered_mechanical_structure_of_the_1D_sensor_(+cad)_model_with_different_parts_colored_separately,_and_universal_magnet_mount_shown_in_green.png})
...
without backlash. This is important if several magnets have to be
measured, where the positioning over the sensor must always be the same.
...
Pico} and a \emph{STM32F4} are used as an computer-sensor interface for
this thesis, which are populare, cost-effective and simple to implement.
...
the sensor \gls{ic}s. These hole are necessary for later adaptions and
...
synchronization and also offers options for later sensor-extensions.
This functionality opens up the possibility of synchronising data from
...
according to the sensors found at startup. If new hardware needs to be
integrated, only the required functions in listing
\ref{lst:CustomSensorClass} need to be implemented.
...
\ref{sensor-syncronisation-interface}. The last step in the setup is to
...
\ref{sensor-syncronisation-interface} and are not out of sequence or
drift.
...
to the user after connecting. The figure
\ref{Result_of_an_executed_b_field_query_using_the_sensors_(+cli)_with_immediate_user_readable_response.png}
shows the current measured value queried by using the \emph{readout}
command by the user.

\begin{figure}
\centering
\includegraphics{./generated_images/border_Result_of_an_executed_b_field_query_using_the_sensors_(+cli)_with_immediate_user_readable_response.png}
\caption{Result of an executed b field query using the sensors \gls{cli}
with immediate user readable response
\label{Result_of_an_executed_b_field_query_using_the_sensors_(+cli)_with_immediate_user_readable_response.png}}
\end{figure}
...
firmware to synchronise the readout intervals. The figure
...
illustrates the schematic how several sensors must be wired together in
order to implement this form of synchronisation.
...
microcontroller with the funtionality to detect if the sensor is a
...
\caption{Build hardware sensors with equipped with different sensor
\gls{ic}s to cover different magnet charaterisation setups
\label{Build_hardware_sensors_with_equipped_with_different_sensor_(+ic)s_to_cover_different_magnet_charaterisation_setups.csv}}\tabularnewline
\toprule
...
The table
\ref{Build_hardware_sensors_with_equipped_with_different_sensor_(+ic)s_to_cover_different_magnet_charaterisation_setups.csv}
...
in the use cases \ref{use-cases}. The most important difference, apart
...
\begin{figure}
\centering
\includegraphics{./generated_images/border_Manufactured_1D_sensor_construction_from_different_angles_and_universal_magnet_mount_(green_3D_printed_part)_populated_with_N45_12x12x12mm_magnet.png}
\caption{Manufactured 1D sensor construction from different angles and
universal magnet mount (green 3D printed part) populated with N45
12x12x12mm magnet
\label{Manufactured_1D_sensor_construction_from_different_angles_and_universal_magnet_mount_(green_3D_printed_part)_populated_with_N45_12x12x12mm_magnet.png}}
\end{figure}

The 1D sensor shown in figure
\ref{Manufactured_1D_sensor_construction_from_different_angles_and_universal_magnet_mount_(green_3D_printed_part)_populated_with_N45_12x12x12mm_magnet.png}
...
screws to minimise possible influences on the measurement.
...
in figure
\ref{Manufactured_1D_sensor_construction_from_different_angles_and_universal_magnet_mount_(green_3D_printed_part)_populated_with_N45_12x12x12mm_magnet.png})over
...
The figure
\ref{3D_plot_of_an_N45_12x12x12mm_magnet_using_the_build_3D_full-sphere_sensor_with_visible_magnetic_field_strength_deviations_using_different_shaded_vertices_on_a_sphere.png}
...
\begin{figure}
\centering
\includegraphics{./generated_images/border_3D_plot_of_an_N45_12x12x12mm_magnet_using_the_build_3D_full-sphere_sensor_with_visible_magnetic_field_strength_deviations_using_different_shaded_vertices_on_a_sphere.png}
\caption{3D plot of an N45 12x12x12mm magnet using the build 3D
full-sphere sensor with visible magnetic field strength deviations using
different shaded vertices on a sphere
\label{3D_plot_of_an_N45_12x12x12mm_magnet_using_the_build_3D_full-sphere_sensor_with_visible_magnetic_field_strength_deviations_using_different_shaded_vertices_on_a_sphere.png}}
\end{figure}
...
\ref{MRP_library_implemented_modules_high_level_overview_each_of_which_can_be_replaced_by_the_user_with_custom_ones.png}.
...
\begin{figure}
\centering
\includegraphics{./generated_images/border_MRP_library_implemented_modules_high_level_overview_each_of_which_can_be_replaced_by_the_user_with_custom_ones.png}
\caption{MRP library implemented modules high level overview each of
which can be replaced by the user with custom ones
\label{MRP_library_implemented_modules_high_level_overview_each_of_which_can_be_replaced_by_the_user_with_custom_ones.png}}
\end{figure}
...
later on. Using the DataAquisition sub-step, it is possible to generate
...
\customcite{O24} project, the function signature of the start function
must be changed. This receives the result of the previous step, in this
case the generated magnet data. By optionally setting meta data in the
universal library data type, constants can be replaced in the analysis
...
to be removed in order to use the added meta data as input parameter
instead.
...
using the library is dumped to the filesystem. It can be seen that by
using the \gls{json} format, all measurement points and metadata are
available in readable plain text.
...
\customcite{HMW20}, or the program for mathematical calculations,
\emph{MATLAB} are often used, the library also supports export formats
for these systems.
...
small footprint and low power consumption make it a good choice. It can
also be used in a temperature chamber.

The \emph{MRPProxy} module in figure
\ref{MRPlib_proxy_module_usage_example_with_host_(+pc)_for_readout_processing_and_remote_(+pc)_for_sensor_communication_communicating_together_using_the_http_protocol.png}
...
\begin{figure}
\centering
\includegraphics{./generated_images/border_MRPlib_proxy_module_usage_example_with_host_(+pc)_for_readout_processing_and_remote_(+pc)_for_sensor_communication_communicating_together_using_the_http_protocol.png}
\caption{MRPlib proxy module usage example with host \gls{pc} for
readout processing and remote \gls{pc} for sensor communication
communicating together using the http protocol
\label{MRPlib_proxy_module_usage_example_with_host_(+pc)_for_readout_processing_and_remote_(+pc)_for_sensor_communication_communicating_together_using_the_http_protocol.png}}
\end{figure}
...
sensors available in the network are accessable to the \emph{control}
\gls{pc}.
...
Figure \ref{Example_MRP_proxy_module_usage,_using_two_remote_(+pc)s.png}
...
Figure \ref{MRPlib_Proxy_Module.png} shows the separation of the various
\gls{hal} instances, which communicate with the physically connected
sensors on the \emph{remote} \gls{pc} and the \emph{control} \gls{pc}
side, which communicates with the remote side via the network. For the
user, nothing changes in the procedure for setting up a measurement. The
\emph{MRPProxy} \gls{cli} application must always be started like shown
in listing \ref{lst:mrpcli_proxy_start} on the \gls{pc} with connected
...
\ref{sensor-syncronisation-interface}. This no longer works for long
distances and the syncronisation must be established via a shared
network connection.
...
Each connected sensor is accessed via the text-based \gls{cli}, which is
...
running \gls{mrp} \gls{cli}, so multiple sensors must be combined into
one virtual one. This is done in several steps, start procedure
...
\begin{lstlisting}[language=bash, caption={MRPproxy REST entry point query examples}, label=lst:mtsc]
...
multi-sensor setup, the command received needs to be forwarded to the
...
are two possible approaches to how the command is processed:
...
which is explained in chapter \ref{network-proxy} by another \gls{uuid}
...
\begin{lstlisting}[language=Python, caption={MRPReading example for setting up a basic measurement}, label=lst:mrpexample_reading]
...
\begin{lstlisting}[language=Python, caption={MRPHal example to use a connected hardware sensor to store readings inside of a measurement}, label=lst:mrpexample_hal]
...
This can be used to construct a Halbach ring magnet (see chapter
...
simplify usage, a pre-defined list of verified functions for use in
...
in figure \ref{Example_measurement_analysis_pipeline.png} as \emph{yaml}
syntax.
...
The various steps are linked by using the \emph{stage } makro as input
parameter of the next function to be executed (see comments in listing
\ref{lst:mrpuddp_example_yaml}).

Therefore it is feasable to use the results of one function in several
others without them directly following each other. The disadvantages of
...
execution sequence can be determined in the form of a call tree in
figure
...
results and the final results in figure
...
vulnerabilities before the software is released as a new version.
...
commonly used package registry and offers direct support for the package
...
The \gls{cli} scripts are written in Python and simplifiys the execution
for the user since there ist no \emph{python3} prefix needed. This has
...
In order to provide comprehensive documentation for the end-user, the
source code is documented using
...
possible methods to the user as an assistance. It is implemented in
listing \ref{lst:pydocstring}.
...
Another possibility for filterin is to use a reference measurement
instead of a simulated ideal magnet as a reference, as described before.
This can initialy come from a magnet selected as a reference magnet. As
...
to be called. As a final step, the result is used in the
...
described that the sensors can still be used for these measurement.
