...
flows in a conductor through which a magnetic field passes. The sensor
...
latencies.\customcite{WSB19} Nevertheless, complex data processing tasks
...
to are inserted into a \emph{heapq} priority queue. As the measured
values are full sphere measurements, the \emph{CoG} in this
implementation is calculated using this formula:
...
The following parameters are determined and analysed for the sensor
evaulation:
...
\(\sigma_{t,tlv}\)=0.53\(^{\circ}\)C indicates a consistent trend. This
...
is expected to be between \emph{20.15\(^{\circ}\)C} and
...
\emph{19.40\(^{\circ}\)C} with an accuracy of two decimal places.
...
\emph{\(\sigma_{rv,tlv}\)=172.0\({\mu}\)T}, which is twice the value
...
\emph{\(\sigma_{rv, mmc}\)=0.20\({\mu}\)T} and is negligible, especially
...
temperature is set to \emph{\(\mu_{trev, 7055BT}\)=21.0\(^{\circ}\)C} in
the measurement runs and thus corresponds to the same conditions as in
the \emph{Background Noise} chapter
...
\midrule
\endhead
\(N_{baseline}\) & 10000 & 10000 & - & Number of samples measured for
baseline run\tabularnewline
\(N_{measurement}\) & 2000 & 2000 & - & Number of samples measured for
measurement run\tabularnewline
\(\mu_{nl}\) & 0.79 & 0.00 & \% & Mean noise level\tabularnewline
\(\sigma_{nl}\) & 0.76 & 0.00 & \% & \gls{sd} noise level\tabularnewline
...
However, due to the limited \gls{dr} of around \emph{±3mT}, direct
...
with a specified \gls{dr} of \emph{±150mT}, but does not achieve the
required accuracy due to strong noise and steep temperature
coefficients. In the following chapter, recommendations for action are
defined, which are derived from the analysis results.
