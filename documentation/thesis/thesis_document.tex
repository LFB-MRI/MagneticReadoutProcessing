...
\tightlist
\item
  Raspberry Pi OS Lite - \passthrough{\lstinline!debian bookworm x64!},
\item
...
separate build configuration and the protocol of the measuring device
was implemented.
...
use sensors that are directly connected to the \gls{pc} with the
...
limited. So multibe sensor can be connected to a
\passthrough{\lstinline!remote!} \gls{pc} which is available on the
network. This can be a \gls{sbc} (e.g.~a Raspberry Pi). The small
