...
homogeneity of \emph{2400 \gls{ppm}} over a \emph{20cm} diameter using
smaller magnets (\(12 x 12 x 12 mm^3\)). This exceptional homogeneity
enables the utilization of coil-based gradients for spatial encoding,
...
\item
  ...
  \gls{ppm}}, with a specific focus on determining the sensors'
  viability for noise measurements.
\end{itemize}
...
is human and machine readable and there is a compatible parser for every
...
\begin{lstlisting}[language=bash, caption={MRPproxy (+rest) capabilities query result after execution of the merging algorithm}, label=lst:mtsc]
...
\begin{lstlisting}[language=Python, caption={MRPHal example to use a connected hardware sensor to store raw sensor readings inside of a measurement}, label=lst:mrpexample_hal]
...
\begin{lstlisting}[language=bash, caption={MRP(+cli) example usage for configuration of a measurement run}, label=lst:mrpcli_config_run]
...
Therefore, it is feasible to use the results of one function in several
