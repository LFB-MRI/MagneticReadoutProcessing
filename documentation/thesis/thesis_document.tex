...
In the realm of modern medical imaging, high-field superconducting
...
The focus of the ongoing efforts is to enhance the homogeneity of
...
This work distinctly prioritizes the development of both hardware and
...
\hypertarget{sensor-characterisation-noise}{%
\subsection{Sensor Characterisation:
Noise}\label{sensor-characterisation-noise}}
...
size of N=10000 measured values was recorded for the baseline
measurement.
...
long-term fluctuations. For this purpose, N=2000 further measured values
were taken with a trigger and readout rate of one measurement per
second.
...
\begin{longtable}[]{@{}lll@{}}
\caption{Sensor noise evaluation result used symbols
\label{Sensor_noise_evaluation_result_used_symbols.csv}}\tabularnewline
\toprule
Symbol & Unit & Description\tabularnewline
\midrule
\endfirsthead
\toprule
Symbol & Unit & Description\tabularnewline
\midrule
\endhead
\(\mu_{rv}\) & \(\mu\)T &\tabularnewline
\(\mu_{t}\) & \(\mu\)T &\tabularnewline
\(\mu_{nl}\) & \% &\tabularnewline
\(\sigma_{nl}\) & \% &\tabularnewline
\bottomrule
\end{longtable}
...
\subsection{Temperature Sensitivity}\label{temperature-sensitivity}}
...
\subsubsection{Result Analysis}\label{result-analysis-1}}
