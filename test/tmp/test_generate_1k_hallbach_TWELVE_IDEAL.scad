projection(cut = true) union(){
    difference(){
        union(){
            union(){
                difference(){
                    cylinder(h=12.0, d=72.0, center=true);
                    cylinder(h=12.001, d=24, center=true);
                };
            }; // create_cylinder_with_cutout_inner_24mm_outer72.0mm_thickness12.0mm
            union(){
                difference(){
                    cylinder(h=12.0, d=71.0, center=true);
                    cylinder(h=12.0, d=77.0, center=true);
                }; // mount_contruction_helper_cylinder
                translate(v=[0, -42.5, 0]){
                    difference(){
                        cube(size=[100, 20, 12.0], center=true); // mount_base
                        union(){
                            translate(v=[40.0, 0, 0]){
                                cylinder(h=12.0, d=10);
                            }; // mount_hole_a
                            translate(v=[-40.0, 0, 0]){
                                cylinder(h=12.0, d=10);
                            }; // mount_hole_a
                        };
                    };
                }; // mount_bar_top
            }; // append_mounting_holes_to_base_slice_10_100
        };
        rotate(a=[0.0, 0.0, 179.29289321881345]){
            translate(v=[24.0, 0.0, 0.0]){
                union(){
                    cube(size=[12.1, 12.1, 12.1], center=true); // magpylib.magnet.Cuboid
                    translate(v=[6.05, 0, -6.0]){
                        cylinder(h=24.0, d=6.05);
                    }; // annotation_cube
                    linear_extrude(height=12.0){
                        rotate(a=[0, 0, 0]){
                            translate(v=[9.0, 6.383333333333333, -12.0]){
                                mirror([1, 0, 0]){
                                    text(text="t2:i0", size=2);
                                };
                            };
                        };
                    }; // annotation_text
                };
            };
        }; // ops_magnet_t2:i0
        rotate(a=[-0.0, -0.0, -90.70710678118655]){
            translate(v=[16.970562748477143, 16.97056274847714, 0.0]){
                union(){
                    cube(size=[12.1, 12.1, 12.1], center=true); // magpylib.magnet.Cuboid
                    translate(v=[6.05, 0, -6.0]){
                        cylinder(h=24.0, d=6.05);
                    }; // annotation_cube
                    linear_extrude(height=12.0){
                        rotate(a=[0, 0, 0]){
                            translate(v=[9.0, 6.383333333333333, -12.0]){
                                mirror([1, 0, 0]){
                                    text(text="t2:i0", size=2);
                                };
                            };
                        };
                    }; // annotation_text
                };
            };
        }; // ops_magnet_t2:i0
        rotate(a=[0.0, 0.0, -0.7071067811865472]){
            translate(v=[1.4695761589768238e-15, 24.0, 0.0]){
                union(){
                    cube(size=[12.1, 12.1, 12.1], center=true); // magpylib.magnet.Cuboid
                    translate(v=[6.05, 0, -6.0]){
                        cylinder(h=24.0, d=6.05);
                    }; // annotation_cube
                    linear_extrude(height=12.0){
                        rotate(a=[0, 0, 0]){
                            translate(v=[9.0, 6.383333333333333, -12.0]){
                                mirror([1, 0, 0]){
                                    text(text="t2:i0", size=2);
                                };
                            };
                        };
                    }; // annotation_text
                };
            };
        }; // ops_magnet_t2:i0
        rotate(a=[0.0, 0.0, 89.29289321881345]){
            translate(v=[-16.97056274847714, 16.970562748477143, 0.0]){
                union(){
                    cube(size=[12.1, 12.1, 12.1], center=true); // magpylib.magnet.Cuboid
                    translate(v=[6.05, 0, -6.0]){
                        cylinder(h=24.0, d=6.05);
                    }; // annotation_cube
                    linear_extrude(height=12.0){
                        rotate(a=[0, 0, 0]){
                            translate(v=[9.0, 6.383333333333333, -12.0]){
                                mirror([1, 0, 0]){
                                    text(text="t2:i0", size=2);
                                };
                            };
                        };
                    }; // annotation_text
                };
            };
        }; // ops_magnet_t2:i0
        rotate(a=[0.0, 0.0, 179.29289321881345]){
            translate(v=[-24.0, 2.9391523179536475e-15, 0.0]){
                union(){
                    cube(size=[12.1, 12.1, 12.1], center=true); // magpylib.magnet.Cuboid
                    translate(v=[6.05, 0, -6.0]){
                        cylinder(h=24.0, d=6.05);
                    }; // annotation_cube
                    linear_extrude(height=12.0){
                        rotate(a=[0, 0, 0]){
                            translate(v=[9.0, 6.383333333333333, -12.0]){
                                mirror([1, 0, 0]){
                                    text(text="t2:i0", size=2);
                                };
                            };
                        };
                    }; // annotation_text
                };
            };
        }; // ops_magnet_t2:i0
        rotate(a=[-0.0, -0.0, -90.70710678118655]){
            translate(v=[-16.970562748477143, -16.97056274847714, 0.0]){
                union(){
                    cube(size=[12.1, 12.1, 12.1], center=true); // magpylib.magnet.Cuboid
                    translate(v=[6.05, 0, -6.0]){
                        cylinder(h=24.0, d=6.05);
                    }; // annotation_cube
                    linear_extrude(height=12.0){
                        rotate(a=[0, 0, 0]){
                            translate(v=[9.0, 6.383333333333333, -12.0]){
                                mirror([1, 0, 0]){
                                    text(text="t2:i0", size=2);
                                };
                            };
                        };
                    }; // annotation_text
                };
            };
        }; // ops_magnet_t2:i0
        rotate(a=[0.0, 0.0, -0.7071067811865472]){
            translate(v=[-4.408728476930472e-15, -24.0, 0.0]){
                union(){
                    cube(size=[12.1, 12.1, 12.1], center=true); // magpylib.magnet.Cuboid
                    translate(v=[6.05, 0, -6.0]){
                        cylinder(h=24.0, d=6.05);
                    }; // annotation_cube
                    linear_extrude(height=12.0){
                        rotate(a=[0, 0, 0]){
                            translate(v=[9.0, 6.383333333333333, -12.0]){
                                mirror([1, 0, 0]){
                                    text(text="t2:i0", size=2);
                                };
                            };
                        };
                    }; // annotation_text
                };
            };
        }; // ops_magnet_t2:i0
        rotate(a=[0.0, 0.0, 89.29289321881345]){
            translate(v=[16.970562748477136, -16.970562748477143, 0.0]){
                union(){
                    cube(size=[12.1, 12.1, 12.1], center=true); // magpylib.magnet.Cuboid
                    translate(v=[6.05, 0, -6.0]){
                        cylinder(h=24.0, d=6.05);
                    }; // annotation_cube
                    linear_extrude(height=12.0){
                        rotate(a=[0, 0, 0]){
                            translate(v=[9.0, 6.383333333333333, -12.0]){
                                mirror([1, 0, 0]){
                                    text(text="t2:i0", size=2);
                                };
                            };
                        };
                    }; // annotation_text
                };
            };
        }; // ops_magnet_t2:i0
        rotate(a=[0.0, 0.0, 179.29289321881345]){
            translate(v=[24.0, 0.0, 0.0]){
                union(){
                    cube(size=[12.1, 12.1, 12.1], center=true); // magpylib.magnet.Cuboid
                    translate(v=[6.05, 0, -6.0]){
                        cylinder(h=24.0, d=6.05);
                    }; // annotation_cube
                    linear_extrude(height=12.0){
                        rotate(a=[0, 0, 0]){
                            translate(v=[9.0, 6.383333333333333, -12.0]){
                                mirror([1, 0, 0]){
                                    text(text="t2:i0", size=2);
                                };
                            };
                        };
                    }; // annotation_text
                };
            };
        }; // ops_magnet_t2:i0
        rotate(a=[-0.0, -0.0, -90.70710678118655]){
            translate(v=[16.970562748477143, 16.97056274847714, 0.0]){
                union(){
                    cube(size=[12.1, 12.1, 12.1], center=true); // magpylib.magnet.Cuboid
                    translate(v=[6.05, 0, -6.0]){
                        cylinder(h=24.0, d=6.05);
                    }; // annotation_cube
                    linear_extrude(height=12.0){
                        rotate(a=[0, 0, 0]){
                            translate(v=[9.0, 6.383333333333333, -12.0]){
                                mirror([1, 0, 0]){
                                    text(text="t2:i0", size=2);
                                };
                            };
                        };
                    }; // annotation_text
                };
            };
        }; // ops_magnet_t2:i0
        rotate(a=[0.0, 0.0, -0.7071067811865472]){
            translate(v=[1.4695761589768238e-15, 24.0, 0.0]){
                union(){
                    cube(size=[12.1, 12.1, 12.1], center=true); // magpylib.magnet.Cuboid
                    translate(v=[6.05, 0, -6.0]){
                        cylinder(h=24.0, d=6.05);
                    }; // annotation_cube
                    linear_extrude(height=12.0){
                        rotate(a=[0, 0, 0]){
                            translate(v=[9.0, 6.383333333333333, -12.0]){
                                mirror([1, 0, 0]){
                                    text(text="t2:i0", size=2);
                                };
                            };
                        };
                    }; // annotation_text
                };
            };
        }; // ops_magnet_t2:i0
        rotate(a=[0.0, 0.0, 89.29289321881345]){
            translate(v=[-16.97056274847714, 16.970562748477143, 0.0]){
                union(){
                    cube(size=[12.1, 12.1, 12.1], center=true); // magpylib.magnet.Cuboid
                    translate(v=[6.05, 0, -6.0]){
                        cylinder(h=24.0, d=6.05);
                    }; // annotation_cube
                    linear_extrude(height=12.0){
                        rotate(a=[0, 0, 0]){
                            translate(v=[9.0, 6.383333333333333, -12.0]){
                                mirror([1, 0, 0]){
                                    text(text="t2:i0", size=2);
                                };
                            };
                        };
                    }; // annotation_text
                };
            };
        }; // ops_magnet_t2:i0
        rotate(a=[0.0, 0.0, 179.29289321881345]){
            translate(v=[-24.0, 2.9391523179536475e-15, 0.0]){
                union(){
                    cube(size=[12.1, 12.1, 12.1], center=true); // magpylib.magnet.Cuboid
                    translate(v=[6.05, 0, -6.0]){
                        cylinder(h=24.0, d=6.05);
                    }; // annotation_cube
                    linear_extrude(height=12.0){
                        rotate(a=[0, 0, 0]){
                            translate(v=[9.0, 6.383333333333333, -12.0]){
                                mirror([1, 0, 0]){
                                    text(text="t2:i0", size=2);
                                };
                            };
                        };
                    }; // annotation_text
                };
            };
        }; // ops_magnet_t2:i0
        rotate(a=[-0.0, -0.0, -90.70710678118655]){
            translate(v=[-16.970562748477143, -16.97056274847714, 0.0]){
                union(){
                    cube(size=[12.1, 12.1, 12.1], center=true); // magpylib.magnet.Cuboid
                    translate(v=[6.05, 0, -6.0]){
                        cylinder(h=24.0, d=6.05);
                    }; // annotation_cube
                    linear_extrude(height=12.0){
                        rotate(a=[0, 0, 0]){
                            translate(v=[9.0, 6.383333333333333, -12.0]){
                                mirror([1, 0, 0]){
                                    text(text="t2:i0", size=2);
                                };
                            };
                        };
                    }; // annotation_text
                };
            };
        }; // ops_magnet_t2:i0
        rotate(a=[0.0, 0.0, -0.7071067811865472]){
            translate(v=[-4.408728476930472e-15, -24.0, 0.0]){
                union(){
                    cube(size=[12.1, 12.1, 12.1], center=true); // magpylib.magnet.Cuboid
                    translate(v=[6.05, 0, -6.0]){
                        cylinder(h=24.0, d=6.05);
                    }; // annotation_cube
                    linear_extrude(height=12.0){
                        rotate(a=[0, 0, 0]){
                            translate(v=[9.0, 6.383333333333333, -12.0]){
                                mirror([1, 0, 0]){
                                    text(text="t2:i0", size=2);
                                };
                            };
                        };
                    }; // annotation_text
                };
            };
        }; // ops_magnet_t2:i0
        rotate(a=[0.0, 0.0, 89.29289321881345]){
            translate(v=[16.970562748477136, -16.970562748477143, 0.0]){
                union(){
                    cube(size=[12.1, 12.1, 12.1], center=true); // magpylib.magnet.Cuboid
                    translate(v=[6.05, 0, -6.0]){
                        cylinder(h=24.0, d=6.05);
                    }; // annotation_cube
                    linear_extrude(height=12.0){
                        rotate(a=[0, 0, 0]){
                            translate(v=[9.0, 6.383333333333333, -12.0]){
                                mirror([1, 0, 0]){
                                    text(text="t2:i0", size=2);
                                };
                            };
                        };
                    }; // annotation_text
                };
            };
        }; // ops_magnet_t2:i0
        rotate(a=[0.0, 0.0, 179.29289321881345]){
            translate(v=[24.0, 0.0, 0.0]){
                union(){
                    cube(size=[12.1, 12.1, 12.1], center=true); // magpylib.magnet.Cuboid
                    translate(v=[6.05, 0, -6.0]){
                        cylinder(h=24.0, d=6.05);
                    }; // annotation_cube
                    linear_extrude(height=12.0){
                        rotate(a=[0, 0, 0]){
                            translate(v=[9.0, 6.383333333333333, -12.0]){
                                mirror([1, 0, 0]){
                                    text(text="t2:i0", size=2);
                                };
                            };
                        };
                    }; // annotation_text
                };
            };
        }; // ops_magnet_t2:i0
        rotate(a=[-0.0, -0.0, -120.70710678118655]){
            translate(v=[20.784609690826528, 11.999999999999998, 0.0]){
                union(){
                    cube(size=[12.1, 12.1, 12.1], center=true); // magpylib.magnet.Cuboid
                    translate(v=[6.05, 0, -6.0]){
                        cylinder(h=24.0, d=6.05);
                    }; // annotation_cube
                    linear_extrude(height=12.0){
                        rotate(a=[0, 0, 0]){
                            translate(v=[9.0, 6.383333333333333, -12.0]){
                                mirror([1, 0, 0]){
                                    text(text="t2:i0", size=2);
                                };
                            };
                        };
                    }; // annotation_text
                };
            };
        }; // ops_magnet_t2:i0
        rotate(a=[-0.0, -0.0, -60.70710678118658]){
            translate(v=[12.000000000000004, 20.784609690826528, 0.0]){
                union(){
                    cube(size=[12.1, 12.1, 12.1], center=true); // magpylib.magnet.Cuboid
                    translate(v=[6.05, 0, -6.0]){
                        cylinder(h=24.0, d=6.05);
                    }; // annotation_cube
                    linear_extrude(height=12.0){
                        rotate(a=[0, 0, 0]){
                            translate(v=[9.0, 6.383333333333333, -12.0]){
                                mirror([1, 0, 0]){
                                    text(text="t2:i0", size=2);
                                };
                            };
                        };
                    }; // annotation_text
                };
            };
        }; // ops_magnet_t2:i0
        rotate(a=[0.0, 0.0, -0.7071067811865472]){
            translate(v=[1.4695761589768238e-15, 24.0, 0.0]){
                union(){
                    cube(size=[12.1, 12.1, 12.1], center=true); // magpylib.magnet.Cuboid
                    translate(v=[6.05, 0, -6.0]){
                        cylinder(h=24.0, d=6.05);
                    }; // annotation_cube
                    linear_extrude(height=12.0){
                        rotate(a=[0, 0, 0]){
                            translate(v=[9.0, 6.383333333333333, -12.0]){
                                mirror([1, 0, 0]){
                                    text(text="t2:i0", size=2);
                                };
                            };
                        };
                    }; // annotation_text
                };
            };
        }; // ops_magnet_t2:i0
        rotate(a=[0.0, 0.0, 59.292893218813454]){
            translate(v=[-11.999999999999996, 20.784609690826528, 0.0]){
                union(){
                    cube(size=[12.1, 12.1, 12.1], center=true); // magpylib.magnet.Cuboid
                    translate(v=[6.05, 0, -6.0]){
                        cylinder(h=24.0, d=6.05);
                    }; // annotation_cube
                    linear_extrude(height=12.0){
                        rotate(a=[0, 0, 0]){
                            translate(v=[9.0, 6.383333333333333, -12.0]){
                                mirror([1, 0, 0]){
                                    text(text="t2:i0", size=2);
                                };
                            };
                        };
                    }; // annotation_text
                };
            };
        }; // ops_magnet_t2:i0
        rotate(a=[0.0, 0.0, 119.29289321881346]){
            translate(v=[-20.784609690826528, 11.999999999999998, 0.0]){
                union(){
                    cube(size=[12.1, 12.1, 12.1], center=true); // magpylib.magnet.Cuboid
                    translate(v=[6.05, 0, -6.0]){
                        cylinder(h=24.0, d=6.05);
                    }; // annotation_cube
                    linear_extrude(height=12.0){
                        rotate(a=[0, 0, 0]){
                            translate(v=[9.0, 6.383333333333333, -12.0]){
                                mirror([1, 0, 0]){
                                    text(text="t2:i0", size=2);
                                };
                            };
                        };
                    }; // annotation_text
                };
            };
        }; // ops_magnet_t2:i0
        rotate(a=[0.0, 0.0, 179.29289321881345]){
            translate(v=[-24.0, 2.9391523179536475e-15, 0.0]){
                union(){
                    cube(size=[12.1, 12.1, 12.1], center=true); // magpylib.magnet.Cuboid
                    translate(v=[6.05, 0, -6.0]){
                        cylinder(h=24.0, d=6.05);
                    }; // annotation_cube
                    linear_extrude(height=12.0){
                        rotate(a=[0, 0, 0]){
                            translate(v=[9.0, 6.383333333333333, -12.0]){
                                mirror([1, 0, 0]){
                                    text(text="t2:i0", size=2);
                                };
                            };
                        };
                    }; // annotation_text
                };
            };
        }; // ops_magnet_t2:i0
        rotate(a=[-0.0, -0.0, -120.70710678118655]){
            translate(v=[-20.784609690826528, -12.000000000000004, 0.0]){
                union(){
                    cube(size=[12.1, 12.1, 12.1], center=true); // magpylib.magnet.Cuboid
                    translate(v=[6.05, 0, -6.0]){
                        cylinder(h=24.0, d=6.05);
                    }; // annotation_cube
                    linear_extrude(height=12.0){
                        rotate(a=[0, 0, 0]){
                            translate(v=[9.0, 6.383333333333333, -12.0]){
                                mirror([1, 0, 0]){
                                    text(text="t2:i0", size=2);
                                };
                            };
                        };
                    }; // annotation_text
                };
            };
        }; // ops_magnet_t2:i0
        rotate(a=[-0.0, -0.0, -60.70710678118658]){
            translate(v=[-12.00000000000001, -20.78460969082652, 0.0]){
                union(){
                    cube(size=[12.1, 12.1, 12.1], center=true); // magpylib.magnet.Cuboid
                    translate(v=[6.05, 0, -6.0]){
                        cylinder(h=24.0, d=6.05);
                    }; // annotation_cube
                    linear_extrude(height=12.0){
                        rotate(a=[0, 0, 0]){
                            translate(v=[9.0, 6.383333333333333, -12.0]){
                                mirror([1, 0, 0]){
                                    text(text="t2:i0", size=2);
                                };
                            };
                        };
                    }; // annotation_text
                };
            };
        }; // ops_magnet_t2:i0
        rotate(a=[0.0, 0.0, -0.7071067811865472]){
            translate(v=[-4.408728476930472e-15, -24.0, 0.0]){
                union(){
                    cube(size=[12.1, 12.1, 12.1], center=true); // magpylib.magnet.Cuboid
                    translate(v=[6.05, 0, -6.0]){
                        cylinder(h=24.0, d=6.05);
                    }; // annotation_cube
                    linear_extrude(height=12.0){
                        rotate(a=[0, 0, 0]){
                            translate(v=[9.0, 6.383333333333333, -12.0]){
                                mirror([1, 0, 0]){
                                    text(text="t2:i0", size=2);
                                };
                            };
                        };
                    }; // annotation_text
                };
            };
        }; // ops_magnet_t2:i0
        rotate(a=[0.0, 0.0, 59.292893218813454]){
            translate(v=[12.000000000000004, -20.784609690826528, 0.0]){
                union(){
                    cube(size=[12.1, 12.1, 12.1], center=true); // magpylib.magnet.Cuboid
                    translate(v=[6.05, 0, -6.0]){
                        cylinder(h=24.0, d=6.05);
                    }; // annotation_cube
                    linear_extrude(height=12.0){
                        rotate(a=[0, 0, 0]){
                            translate(v=[9.0, 6.383333333333333, -12.0]){
                                mirror([1, 0, 0]){
                                    text(text="t2:i0", size=2);
                                };
                            };
                        };
                    }; // annotation_text
                };
            };
        }; // ops_magnet_t2:i0
        rotate(a=[0.0, 0.0, 119.29289321881346]){
            translate(v=[20.78460969082652, -12.00000000000001, 0.0]){
                union(){
                    cube(size=[12.1, 12.1, 12.1], center=true); // magpylib.magnet.Cuboid
                    translate(v=[6.05, 0, -6.0]){
                        cylinder(h=24.0, d=6.05);
                    }; // annotation_cube
                    linear_extrude(height=12.0){
                        rotate(a=[0, 0, 0]){
                            translate(v=[9.0, 6.383333333333333, -12.0]){
                                mirror([1, 0, 0]){
                                    text(text="t2:i0", size=2);
                                };
                            };
                        };
                    }; // annotation_text
                };
            };
        }; // ops_magnet_t2:i0
    };
};

// 0_create_cylinder_with_cutout_inner_24mm_outer72.0mm_thickness12.0mm 
// 1_ops_magnet_t2:i0 
// 2_ops_magnet_t2:i0 
// 3_ops_magnet_t2:i0 
// 4_ops_magnet_t2:i0 
// 5_ops_magnet_t2:i0 
// 6_ops_magnet_t2:i0 
// 7_ops_magnet_t2:i0 
// 8_ops_magnet_t2:i0 
// 9_create_cylinder_with_cutout_inner_24mm_outer72.0mm_thickness12.0mm 
// 10_ops_magnet_t2:i0 
// 11_ops_magnet_t2:i0 
// 12_ops_magnet_t2:i0 
// 13_ops_magnet_t2:i0 
// 14_ops_magnet_t2:i0 
// 15_ops_magnet_t2:i0 
// 16_ops_magnet_t2:i0 
// 17_ops_magnet_t2:i0 
// 18_ops_magnet_t2:i0 
// 19_ops_magnet_t2:i0 
// 20_ops_magnet_t2:i0 
// 21_ops_magnet_t2:i0 
// 22_ops_magnet_t2:i0 
// 23_ops_magnet_t2:i0 
// 24_ops_magnet_t2:i0 
// 25_ops_magnet_t2:i0 
// 26_ops_magnet_t2:i0 
// 27_ops_magnet_t2:i0 
// 28_ops_magnet_t2:i0 
// 29_ops_magnet_t2:i0 
