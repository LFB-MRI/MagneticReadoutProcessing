...
class MRPHalKlipper(MRPHal.MRPHal):
    ...
    def init(self, _use_dummy: bool = False):

            self.configfile: dict = {}
            if not _use_dummy:
                self. configfile = self.get('/printer/objects/query?configfile')
            else:
                self.configfile = self.DUMMY_CONFIG # in general not needed can be removed, just for testing
