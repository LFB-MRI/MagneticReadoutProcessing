...
class MRPOpenSCADGenerator():
    ...
    def create_magnet_cutout(self, _magnet: magpylib.magnet, _magnet_trajectory: float, _rotation_drg_x:float,_rotation_spacing:float, _annotation: str= None, _safety_margin_mm: float = 0.05):
        if _magnet is None:
        ...
        if isinstance(_magnet, magpylib.magnet.Cuboid):
            ...
            cube= ops.Cube(size=dim,center=True).translate(pos).rotate(rot)
            ...
            max_w = max(dim)
            cube.append(ops.Cylinder(d=max([max_w/3, 2]), h=self.BASE_SLICE_THICKNESS).translate([_magnet_trajectory+ dim[0]/2,0,0]).comment("annotation_cube_d{}_h{}".format(max_w, self.BASE_SLICE_THICKNESS)))

            # ADD INFORMATION TEXT
            cube.append(ops.Linear_Extrude(self.BASE_SLICE_THICKNESS).append(ops.Text("123", size=50)))
